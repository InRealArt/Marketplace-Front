 ...
 */
export async function upsertUserCart(userId: string, items: CartItem[]) {
  try {
    console.log(`Updating user cart (userId: ${userId}) with ${items.length} items`);
    ...
    if (existingCart) {
      ...
      await prisma.$executeRaw`
        ...
        SET "items" = ${JSON.stringify(items)}::jsonb, "updatedAt" = NOW() 
        WHERE "userId" = ${userId}
      `;
    } else {
      ...
      // Create new cart
      await prisma.$executeRaw`
        INSERT INTO "marketplace"."Cart" ("userId", "items", "createdAt", "updatedAt")
        VALUES (${userId}, ${JSON.stringify(items)}::jsonb, NOW(), NOW())
      `;
    }
