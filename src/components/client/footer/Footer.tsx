...
function Footer() {
  ...
  return (
    <footer className="py-8 bg-grey mt-[6rem] w-full text-white">
      <div className=" p-6 sm:p-8 lg:p-12 xl:p-16">
        ...
        {/* Separator */}
        <hr className="bg-white border-0 h-px mb-6" />
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
          <p className="font-montserrat text-sm sm:text-base font-medium">
            © 2025 Inrealart
          </p>
            ...
            <ul className="flex flex-col sm:flex-row gap-2 sm:gap-4 items-start sm:items-center">
              {footerLinks.map((link, index) => (
                <li key={index} className="flex items-center gap-4">
                  <a
                    href={link.href}
                    className="font-montserrat text-sm sm:text-base font-medium hover:underline focus:outline-none focus:ring-2 focus:ring-[#4231ff] focus:ring-offset-2 rounded"
                  >
                    {link.text}
                  </a>
                  {index < footerLinks.length - 1 && (
                    <span
                      className="hidden sm:block bg-white rounded-full w-2 h-2"
                      aria-hidden="true"
                    />
                  )}
                </li>
